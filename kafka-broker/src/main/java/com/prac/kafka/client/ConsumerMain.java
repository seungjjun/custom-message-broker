...
public class ConsumerMain {
    ...
    public static void main(String[] args) throws Exception {
        ...
        FetchResponse res1 = client.fetch("payments", 0, 10);
        log.info("fetch 1 - {}개 수신, nextOffset: {}", res1.records().size(), res1.nextOffset());
        for (FetchRecord record : res1.records()) {
            log.info("  offset={}, key={}, value={}", record.offset(), record.key(), record.value());
        }

        FetchResponse res2 = client.fetch("payments", 0, 10);
        log.info("fetch 2 - {}개 수신 (불변성 확인)", res2.records().size());

        FetchResponse res3 = client.fetch("payments", 2, 10);
        log.info("fetch 3 - offset=2부터 {}개 수신", res3.records().size());
